...
import { validateExtractedFieldsForScan } from "../../features/id";
...
import { cleanNameCandidate, } from "../../utils/idParsers";
    ...
    try {
      ...
      function isReasonableName(candidate: string): boolean {
        ...
        return true;
            ...
            const visionName = cleanNameCandidate(visionResult.fullName || "");
      const roiName = cleanNameCandidate(roiFullName || "");
      const fullNameFromFull = cleanNameCandidate(fromFull.fullName || "");

      const useVisionName = !!visionName;
      const useRoiName = !!roiName && isReasonableName(roiName);
      ...
      // Priority: Vision > ROI > Full-card Tesseract
        ...
        useVisionName
          ...
          : selectedIdType === "National ID"
            ? (fullNameFromFull || roiName || "")
            : (useRoiName ? roiName : (fullNameFromFull || "")) || "";
  ...
  const handleSubmitVisitor = async () => {
    ...
    try {
      ...
      if (!validation.ok) {
        ...
        showToast(`Please double-check the following field(s): ${joined}.`, {
          variant: "warning",
        });
        setSubmitting(false);
        return;
