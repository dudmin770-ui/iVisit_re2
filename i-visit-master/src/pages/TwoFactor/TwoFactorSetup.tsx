...
const QR_SERVICE_URL =
    (import.meta as any).env?.VITE_QR_SERVICE_URL ||
...
export default function TwoFactorSetup() {
  Meta({ title: "2FA Setup - iVisit" });

  const location = useLocation();
  const navigate = useNavigate();
  const [, setCookie] = useCookies(["role", "username", "stationId", "userId"]);

  const state = location.state as SetupState | undefined;
  console.log("TwoFactorSetup state:", state);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center text-white">
        <div className="bg-white/10 p-6 rounded-xl">
          <p className="mb-4">Missing 2FA setup context.</p>
  ...
  const { userId, email, stationId, otpauthUrl } = state;

  const qrBase = QR_SERVICE_URL.endsWith("/")
    ? QR_SERVICE_URL.slice(0, -1)
    : QR_SERVICE_URL;

  const qrSrc = otpauthUrl
    ? `${qrBase}/?size=200x200&data=${encodeURIComponent(otpauthUrl)}`
    : "";
  ...
  return (
    <div className="min-h-screen p-4 grid place-items-center bg-[url(/background.png)] bg-no-repeat bg-cover">
      <div className="bg-black/60 text-white p-8 rounded-xl max-w-md w-full space-y-4">
        <h1 className="text-2xl font-semibold text-center">Set up 2FA</h1>
        ...
        {qrSrc ? (
          <div className="flex justify-center my-4">
            <img src={qrSrc} alt="2FA QR Code" className="w-48 h-48" />
          </div>
        ) : (
          <p className="text-center text-red-300">
            QR code data missing. Please try signing in again.
          </p>
        )}
